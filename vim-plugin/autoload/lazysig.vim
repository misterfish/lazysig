...
let s:types = ['fil', 'buf']
if !exists('g:fprompt_types')
	let g:fprompt_types = s:types
el
	call filter(g:fprompt_types, "index(['fil', 'buf'], v:val)!=-1")
en
let g:fprompt_builtins = len(g:fprompt_types)-1
...
let s:coretypes = map(copy(g:fprompt_types), '[s:coretype_names[v:val], v:val]')
...
fu! s:opts(...)
	...
	if a:0 && a:1 != {}
		...
		for [ke, va] in items(a:1)
			let opke = substitute(ke, '\(\w:\)\?fprompt_', '', '')
		...
		endfo
	en
	if !exists('g:fprompt_tilde_homedir') | let g:fprompt_tilde_homedir = 0 | en
	if !exists('g:fprompt_newcache') | let g:fprompt_newcache = 0 | en
	...
	if !( exists('extensions') && extensions == s:extensions )
		for each in s:extensions
			exe 'ru autoload/fprompt/'.each.'.vim'
		endfo
	en
endf
...
fu! s:Open()
	...
	if s:opmul != '0' && has('signs')
		sign define fpromptmark text=+> texthl=FPromptMark
...
fu! s:Update()
	let lines = copy(g:fprompt_lines)
	cal s:Render(lines)
" 	return []
endf
...
fu! s:dircompl(be, sd)
	...
	if a:be == ''
		let dirs = fprompt#rmbasedir(dirs)
	en
...
fu! fprompt#igncwd(cwd)
	retu fprompt#utils#glob(a:cwd, 0) == '' ||
...
" Fallbacks 
fu! s:glbpath(...)
	retu call('fprompt#utils#globpath', a:000)
endf

fu! s:fnesc(...)
	retu call('fprompt#utils#fnesc', a:000)
endf

fu! fprompt#setlcdir()
	if exists('*haslocaldir')
		cal fprompt#setdir(getcwd(), haslocaldir() ? 'lc!' : 'cd!')
	en
endf
" Highlighting 
fu! fprompt#syntax()
	if fprompt#nosy() | retu | en
	for [ke, va] in items(s:hlgrps) | cal fprompt#hicheck('FPrompt'.ke, va) | endfo
	...
	if hlexists('FPromptLinePre')
		exe "sy match FPromptLinePre '^".escape(get(g:, 'fprompt_line_prefix', '>'),'^$.*~\')."'"
	en
endf
...
fu! s:dohighlight()
	retu s:mathi[0] && exists('*clearmatches') && !fprompt#nosy()
endf
" Prompt history 
fu! s:gethistloc()
	let utilcadir = fprompt#utils#cachedir()
	let cache_dir = utilcadir.s:lash(utilcadir).'hist'
	retu [cache_dir, cache_dir.s:lash(cache_dir).'cache.txt']
endf

fu! s:gethistdata()
	retu fprompt#utils#readfile(s:gethistloc()[1])
endf

fu! fprompt#recordhist()
	let str = join(s:prompt, '')
	...
	cal fprompt#utils#writecache(hst, s:gethistloc()[0], s:gethistloc()[1])
endf
...
fu! s:log(m)
	if exists('g:fprompt_log') && g:fprompt_log | if a:m
		let cadir = fprompt#utils#cachedir()
		let apd = g:fprompt_log > 1 ? '>' : ''
		sil! exe 'redi! >'.apd cadir.s:lash(cadir).'fprompt.log'
	el
		sil! redi END
	en | en
endf
...
fu! s:execextvar(key)
	if !empty(g:fprompt_ext_vars)
		cal map(filter(copy(g:fprompt_ext_vars),
			\ 'has_key(v:val, a:key)'), 'eval(v:val[a:key])')
	en
endf

fu! s:getextvar(key)
	if s:itemtype >= len(s:coretypes) && len(g:fprompt_ext_vars) > 0
		let vars = g:fprompt_ext_vars[s:itemtype - len(s:coretypes)]
		if has_key(vars, a:key)
			retu vars[a:key]
		en
	en
	retu get(g:, 'fprompt_' . s:matchtype . '_' . a:key, -1)
endf
...
fu! fprompt#hicheck(grp, defgrp)
	if !hlexists(a:grp)
		exe 'hi link' a:grp a:defgrp
	en
endf

fu! fprompt#call(func, ...)
	retu call(a:func, a:000)
endf

fu! fprompt#getvar(var)
	retu {a:var}
endf
...
" Returns [lname, sname]
fu! s:CurTypeName()
    return filter(copy(s:coretypes), 'v:val[1]==g:fprompt_types[s:itemtype]')[0]
endfu

fu! fprompt#init(...)
    " XXX
    let s:parserbin = '/home/fritz/fiddle/hsig/hsig'
	...
	cal s:MapNorms()
	cal s:MapSpecs()
	if empty(g:fprompt_types) && empty(g:fprompt_ext_vars)
		call fprompt#exit()
		retu
	en
	cal fprompt#setlines()
